    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analytics.summary.totalWordCount.toLocaleString()}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analytics.writingStreak} days</div>
            <p className="text-xs text-muted-foreground mt-1">
              {analytics.writingStreak > 0 ? 'Keep it up!' : 'Start writing to build your streak'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{analytics.averageWordsPerDay}</div>
            <p className="text-xs text-muted-foreground mt-1">
              {analytics.averageWordsPerDay > 0 ? 'Your daily writing average' : 'No entries yet'}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      {/* Sentiment Analysis */}
      {analytics.summary.sentiment && analytics.summary.totalEntries > 0 && (
        <div className="space-y-4">
          <h2 className="text-2xl font-bold">Sentiment Analysis</h2>
          <SentimentAnalysis sentiment={analytics.summary.sentiment} />
        </div>
      )}
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                {analytics.summary.longestEntry.wordCount} words • {format(new Date(analytics.summary.longestEntry.createdAt), 'MMM d, yyyy')}
              </p>
            </div>
          </CardContent>
        </Card>
